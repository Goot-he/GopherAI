...
export default {
  ...
  setup() {
    ...
    const createNewSession = () => {
      // 如果已经有一个未开始的临时会话，不再重复创建
      if (tempSession.value && currentSessionId.value === 'temp') return

      const tempId = 'temp'
      sessions.value[tempId] = {
        id: tempId,
        name: '新聊天...',
        messages: []
      }
    ...
    const switchSession = async (sessionId) => {
      ...
      // 如果从 temp 切换走，且 temp 没发过消息，就清理掉它
      if (currentSessionId.value === 'temp' && sessionId !== 'temp') {
        if (currentMessages.value.length === 0) {
          delete sessions.value['temp']
        }
      }

      currentSessionId.value = String(sessionId)
      tempSession.value = (sessionId === 'temp')
                  ...
                  if (parsed.sessionId) {
                    ...
                    if (tempSession.value) {
                      ...
                      sessions.value[newSid] = {
                        id: newSid,
                        name: question.substring(0, 15) + (question.length > 15 ? '...' : ''),
