...
func NewOpenAIModel(ctx context.Context) (*OpenAIModel, error) {
	...
	key := conf.RagModelConfig.OpenAiKey
...
func (o *AliRAGModel) GenerateResponse(ctx context.Context, messages []*schema.Message) (*schema.Message, error) {
	...
	if err != nil {
		log.Printf("[RAG] NewRAGQuery failed for user=%s: %v", o.username, err)
		log.Println("[RAG] Falling back to direct LLM call without retrieval")
		resp, err := o.llm.Generate(ctx, messages)
		if err != nil {
...
func (o *AliRAGModel) StreamResponse(ctx context.Context, messages []*schema.Message, cb StreamCallback) (string, error) {
	...
	if err != nil {
		log.Printf("[RAG] NewRAGQuery failed for user=%s: %v", o.username, err)
		log.Println("[RAG] Falling back to direct LLM stream without retrieval")
...
func NewWebSearchModel(ctx context.Context) (*WebSearchModel, error) {
	key := config.GetConfig().OpenAiKey
	conf := config.GetConfig()
	modelName := conf.RagModelConfig.RagChatModelName
	baseURL := conf.RagModelConfig.RagBaseUrl
	...
	if err != nil {
		return nil, fmt.Errorf("create web search model failed: %v", err)
	}
	return &WebSearchModel{llm: llm}, nil
}

func (w *WebSearchModel) GenerateResponse(ctx context.Context, messages []*schema.Message) (*schema.Message, error) {
	if len(messages) == 0 {
		return nil, fmt.Errorf("no messages provided")
	}
	...
	results, err := search.Search(query, 5)
	if err != nil {
	...
	log.Printf("[WebSearch] Results:\n%s", searchContext)
	prompt := w.buildFinalPrompt(query, searchContext)
	searchMessages := make([]*schema.Message, len(messages))
	copy(searchMessages, messages)
	searchMessages[len(searchMessages)-1] = &schema.Message{
		Role:    schema.User,
		Content: prompt,
	}

	resp, err := w.llm.Generate(ctx, searchMessages)
	if err != nil {
		return nil, fmt.Errorf("search generate failed: %v", err)
	}

	return resp, nil
}

func (w *WebSearchModel) StreamResponse(ctx context.Context, messages []*schema.Message, cb StreamCallback) (string, error) {
	if len(messages) == 0 {
		return "", fmt.Errorf("no messages provided")
	}
	...
	return w.streamFromLLM(ctx, searchMessages, cb)
}

func (w *WebSearchModel) streamFromLLM(ctx context.Context, messages []*schema.Message, cb StreamCallback) (string, error) {
	stream, err := w.llm.Stream(ctx, messages)
	if err != nil {
...
func (w *WebSearchModel) buildFinalPrompt(query, searchContext string) string {
	return fmt.Sprintf(`你是一个智能助手。请基于以下搜索结果回答用户问题。
...
要求：
1. 基于搜索结果提供准确、全面的回答
2. 如果搜索结果中没有相关信息，请如实告知
3. 可以引用具体的结果编号来佐证观点

请回答：`, searchContext, query)
}

func (w *WebSearchModel) GetModelType() string { return "5" }
