...
func NewRAGIndexer(filename, embeddingModel string) (*RAGIndexer, error) {
	...
	indexerConfig := &redisIndexer.IndexerConfig{
		...
		DocumentToHashes: func(ctx context.Context, doc *schema.Document) (*redisIndexer.Hashes, error) {
			...
			// 索引器的 KeyPrefix 已经包含了前缀，这里只需要返回文档 ID
			return &redisIndexer.Hashes{
				// Redis 最终存储的 Key 会是: KeyPrefix + Key
				Key: doc.ID,
...
func (r *RAGIndexer) IndexFile(ctx context.Context, filePath string) error {
	...
	log.Printf("Indexing %d chunks from file: %s", len(docs), filePath)
...
func NewRAGQuery(ctx context.Context, username string) (*RAGQuery, error) {
	...
	if err != nil || len(files) == 0 {
		return nil, fmt.Errorf("no uploaded file found for user %s (dir=%s, err=%v)", username, userDir, err)
	}
	...
	log.Printf("[RAG] Creating retriever for user=%s filename=%s index=%s", username, filename, indexName)
...
func (r *RAGQuery) RetrieveDocuments(ctx context.Context, query string) ([]*schema.Document, error) {
	log.Printf("[RAG] Searching for query: %s", query)
	docs, err := r.retriever.Retrieve(ctx, query)
	if err != nil {
		return nil, fmt.Errorf("failed to retrieve documents: %w", err)
	}
	log.Printf("[RAG] Found %d relevant documents", len(docs))
	for i, d := range docs {
		log.Printf("[RAG] Result %d: %s (Score/Distance: %v)", i+1, d.Content, d.MetaData["distance"])
	}
	return docs, nil
}
