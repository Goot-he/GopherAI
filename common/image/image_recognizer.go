...
// MobileNetV2 标准化参数（ImageNet 数据集的 mean 和 std）
var (
...
func (r *ImageRecognizer) PredictFromImage(img image.Image) (string, error) {
	...
	for y := 0; y < h; y++ {
		for x := 0; x < w; x++ {
			...
			// 先做 [0,1] 缩放，再做标准化 (value - mean) / std
			rf := (float32(r>>8)/255.0 - mobilenetMean[0]) / mobilenetStd[0]
			gf := (float32(g>>8)/255.0 - mobilenetMean[1]) / mobilenetStd[1]
			bf := (float32(b>>8)/255.0 - mobilenetMean[2]) / mobilenetStd[2]
...
func setOnnxRuntimeDLLPath() {
	...
	if wd, err := os.Getwd(); err == nil {
		candidates = append(candidates, filepath.Join(wd, dllName))
	}
